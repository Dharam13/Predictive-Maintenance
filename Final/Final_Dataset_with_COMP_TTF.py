...
def calculate_ttf_advanced_fixed(df, maint_df, fail_df):
    """
    ...
    FIXED: Remove telemetry records where the next failure occurs after an intervening maintenance event.
    """
    results = []
    removed_count = 0
        ...
        for idx, tele_row in machine_tele.iterrows():
            ...
            for comp in components:
                ...
                if len(future_failures) > 0:
                    ...
                    # CRITICAL FIX: Check if there's any maintenance between current time and next failure
                    intervening_maint = comp_maint[
                        (comp_maint['datetime'] > current_time) &
                        (comp_maint['datetime'] < next_failure_time)
                        ]

                    if len(intervening_maint) > 0:
                        # There's maintenance between now and the failure, so this record should be removed
                        should_remove_record = True
                        break  # No need to check other components for this record

                    # Also check if the failure is after the most recent maintenance
                    # (this ensures we only consider failures that are truly "next" after the last reset)
                    if pd.notna(last_maint_time) and next_failure_time <= last_maint_time:
                        # This failure happened before or at the same time as the last maintenance
                        # Look for failures after the last maintenance
                        future_failures_after_maint = comp_fail[
                            (comp_fail['datetime'] > current_time) &
                            (comp_fail['datetime'] > last_maint_time)
                            ]

                        if len(future_failures_after_maint) > 0:
                            next_failure_time = future_failures_after_maint['datetime'].min()

                            # Check again for intervening maintenance
                            intervening_maint = comp_maint[
                                (comp_maint['datetime'] > current_time) &
                                (comp_maint['datetime'] < next_failure_time)
                                ]
                            ...
                            # No future failure after last maintenance, use end of observation period
                            max_time = df['datetime'].max()
                            ttf_hours = (max_time - current_time).total_seconds() / 3600
                    else:
                        # Normal case: calculate TTF to next failure
                        ttf_hours = (next_failure_time - current_time).total_seconds() / 3600
                else:
            ...
            # If this record should be removed, skip it
    ...
    print(f"\nRemoved {removed_count} telemetry records due to intervening maintenance events")
    return pd.DataFrame(results)


# Apply the fixed advanced TTF calculation
print("Calculating fixed advanced TTF...")
df_final = calculate_ttf_advanced_fixed(df2, maint, fail)
...
# Save to CSV file
output_filename = 'data/Fixed_Advanced_TTF_Dataset.csv'
